...
import { getPyodide, runMotorSimulation, exportRicYaml, importRicYaml } from './src/lib/openmotor-pyodide.js'
...
function titleCase(key) { return key.replace(/([A-Z])/g, ' $1').replace(/^./, c => c.toUpperCase()) }

function grainFields(grain) {
  return Object.entries(grain.properties).map(([key, value]) => {
    if (typeof value === 'boolean') return `<label class="field"><span>${titleCase(key)}</span><input data-scope="grain" data-key="${key}" type="checkbox" ${value ? 'checked' : ''}></label>`
    if (key === 'inhibitedEnds') return `<label class="field"><span>${titleCase(key)}</span><select data-scope="grain" data-key="${key}">${allInhibited.map(opt => `<option ${opt===value?'selected':''}>${opt}</option>`).join('')}</select></label>`
    return `<label class="field"><span>${titleCase(key)}</span><input data-scope="grain" data-key="${key}" type="number" step="any" value="${value}"></label>`
  }).join('')
}

function scalarFields(obj, scope, textKeys = []) {
  return Object.entries(obj).map(([key, value]) => {
    if (Array.isArray(value) || typeof value === 'object') return ''
    const type = textKeys.includes(key) ? 'text' : typeof value === 'string' ? 'text' : 'number'
    const step = type === 'number' ? 'step="any"' : ''
    return `<label class="field"><span>${titleCase(key)}</span><input data-scope="${scope}" data-key="${key}" type="${type}" ${step} value="${value}"></label>`
  }).join('')
}
  ...
  app.innerHTML = `
    <div class="shell">
      <header class="topbar">
        <div>
          <h1>openMotor Web</h1>
          <p class="muted">${state.fileName} · ${state.status}</p>
        </div>
      ...
      <div class="content">
        <aside class="sidebar">
          ...
          <div class="panel">
            ...
            <div class="grain-list">
              ${state.motor.grains.map((g, i) => `<button class="grain-item ${i===state.selectedGrain?'active':''}" data-grain-index="${i}">${i + 1}. ${g.type}</button>`).join('')}
            </div>
            <div class="row wrap">
        ...
        <main class="main">
          ...
          <section class="stats-grid">
            ${[
              ['Designation', stats.designation], ['Impulse (Ns)', fmt(stats.impulse)], ['ISP (s)', fmt(stats.isp)], ['Burn time (s)', fmt(stats.burnTime)], ['Volume loading (%)', fmt(stats.volumeLoading)],
              ['Avg pressure (Pa)', fmt(stats.avgPressure, 0)], ['Peak pressure (Pa)', fmt(stats.peakPressure, 0)], ['Initial Kn', fmt(stats.initialKn)], ['Peak Kn', fmt(stats.peakKn)], ['Propellant mass (kg)', fmt(stats.propellantMass)],
          ...
          <section class="panel results-layout">
            ...
            <div>
              ...
              <pre class="json-preview">${JSON.stringify(state.motor, null, 2)}</pre>
            </div>
          </section>
...
async function saveRic() {
  state.status = 'Exporting .ric…'; render()
  const exported = await exportRicYaml(state.motor)
  if (exported.error) {
    state.result = exported
    state.status = 'Export failed.'
    render(); return
  }
  const blob = new Blob([exported.yaml], { type: 'application/x-yaml' })
  const url = URL.createObjectURL(blob)
  const a = document.createElement('a')
  a.href = url
  a.download = state.fileName || 'motor.ric'
  a.click()
  URL.revokeObjectURL(url)
  state.status = 'Saved .ric file.'
  render()
}

async function loadRic(file) {
  const text = await file.text()
  state.status = 'Importing .ric…'; render()
  const imported = await importRicYaml(text)
  if (imported.error) {
    state.result = imported
    state.status = 'Import failed.'
  } else {
    state.motor = imported.motor
    state.fileName = file.name || 'imported.ric'
    state.selectedGrain = 0
    state.result = null
    state.status = 'Imported .ric file.'
  }
  render()
}
...
function bindEvents() {
  ...
  document.getElementById('new-motor').onclick = () => { state.motor = structuredClone(defaultMotor); state.selectedGrain = 0; state.result = null; state.fileName = 'untitled.ric'; state.status = 'Reset.'; render() }
  ...
  document.getElementById('add-grain').onclick = () => { const type = document.getElementById('add-grain-type').value; state.motor.grains.push({ type, properties: structuredClone(grainTemplates[type]) }); state.selectedGrain = state.motor.grains.length - 1; render() }
  document.getElementById('dup-grain').onclick = () => { const g = state.motor.grains[state.selectedGrain]; state.motor.grains.splice(state.selectedGrain + 1, 0, structuredClone(g)); state.selectedGrain++; render() }
  document.getElementById('del-grain').onclick = () => { state.motor.grains.splice(state.selectedGrain, 1); state.selectedGrain = Math.max(0, state.selectedGrain - 1); render() }
...
async function boot() {
  render()
  try { await getPyodide(); state.status = 'Pyodide ready.' } catch { state.status = 'Pyodide failed to load.' }
  render()
}
boot()
